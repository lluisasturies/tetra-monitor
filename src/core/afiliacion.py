...
class AfiliacionConfig:
    ...
    def _load(self):
        if self._filepath.exists():
            try:
                ...
                self.gssi      = afiliacion.get("gssi", "")
                self.scan_list = afiliacion.get("scan_list", "")
                self._last_mtime = self._filepath.stat().st_mtime
                logger.info("Afiliacion config cargada correctamente")
    ...
    def reload_if_changed(self) -> bool:
        ...
        if changed:
            logger.info(
                f"[AfiliacionConfig] Cambio detectado — gssi: '{prev_gssi}'->'{self.gssi}', "
    ...
    def save(self):
        try:
            with open(self._filepath, "w") as f:
                yaml.safe_dump({"afiliacion": {"gssi": self.gssi, "scan_list": self.scan_list}}, f)
            self._last_mtime = self._filepath.stat().st_mtime
