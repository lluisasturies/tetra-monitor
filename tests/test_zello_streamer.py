...
# Simula que el encoder devuelve bytes al llamar a encode()
...
def streamer():
    ...
    with mock.patch("threading.Thread") as mock_thread:
        ...
        # run_coroutine_threadsafe devuelve un future con result() = stream_id
...
def test_importacion_falla_sin_deps():
    """Si websockets/opuslib no estan instalados, ZelloStreamer lanza RuntimeError."""
    ...
    try:
        with pytest.raises(RuntimeError, match="Dependencias de Zello"):
            ZelloStreamer(
                username="u", password="p", token="t", channel="c"
            )
    finally:
...
def test_call_start_no_abre_si_ya_en_llamada(streamer):
    ...
    streamer.call_start()
    # No debe sobrescribir el stream_id existente
...
def test_send_audio_codifica_frame_completo(streamer):
    ...
    streamer._in_call = True
    # Un frame exacto de OPUS_FRAME_SIZE muestras
...
def test_send_audio_acumula_buffer_si_frame_incompleto(streamer):
    ...
    streamer._in_call = True
    streamer._buf     = b""
    # Menos de un frame completo -> no debe encodear todavia
    audio = np.ones(100, dtype=np.float32) * 0.5
    streamer.send_audio(audio)
    mock_encoder_instance.encode.assert_not_called()
    assert len(streamer._buf) == 100 * 2  # 2 bytes por muestra PCM16
...
def test_stop_no_falla_sin_llamada_activa(streamer):
    streamer._in_call = False
    streamer.stop()  # no debe lanzar


# ---------------------------------------------------------------------------
# create_streamer — integracion con __init__.py
# ---------------------------------------------------------------------------

def test_create_streamer_zello_prioridad_sobre_rtmp():
    """Zello tiene prioridad sobre RTMP e Icecast si enabled=True."""
    with mock.patch.dict(os.environ, {
        ...
        with mock.patch("threading.Thread"):
            with mock.patch("streaming.zello_streamer._ZELLO_DEPS_AVAILABLE", True):
                ...
                streamer = create_streamer(cfg)
                assert isinstance(streamer, ZelloStreamer)


def test_create_streamer_zello_desactivado_usa_rtmp():
    """Si zello.enabled=False, debe usar RTMP."""
    from streaming import create_streamer
...
def test_create_streamer_zello_sin_credenciales_devuelve_none():
    """Si zello.enabled=True pero faltan credenciales, debe devolver None."""
    with mock.patch.dict(os.environ, {
        "ZELLO_USERNAME": "",
        "ZELLO_PASSWORD": "",
        "ZELLO_TOKEN":    "",
